...
public class Ellip extends HinhVe {

    {
        ...
        this.hinh = new Ellipse();
        this.hinh.setId("HinhEllipse");
        ...
        for (int i = 0; i < soDiemDieuKhien; i++) {
         ...
         ((Ellipse) hinh).addEventHandler(MouseEvent.MOUSE_PRESSED, new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent e) {
                if (isDoiMauNen) {
                    System.out.println("Hinh Tron");
                    ((Ellipse) hinh).setFill(mauNen);
                }
                 if (isDoiMauNetVe == true) {
                    ((Ellipse) hinh).setStroke(mauNetVe);
                }
            }
        });
    }
    ...
    public Ellip(double netVe, Color mauNetVe, Color mauNen) {
        this.netVe = netVe;
    ...
    public void capNhat() {
        if (viTriChuot != -1) {
            ((Ellipse) (hinh)).setStroke(mauNetVe);
            ((Ellipse) (hinh)).setStrokeWidth(netVe);
        }
    }

    /*Cap nhat thong tin va ve hinh ellipse*/
    @Override
    public void veHinh(double x, double y, double w, double h) {
        ...
        ((Ellipse) (hinh)).setCenterX(x + w / 2);
        ((Ellipse) (hinh)).setCenterY(y + h / 2);
        ((Ellipse) (hinh)).setRadiusX(w / 2);
        ((Ellipse) (hinh)).setRadiusY(h / 2);
    }

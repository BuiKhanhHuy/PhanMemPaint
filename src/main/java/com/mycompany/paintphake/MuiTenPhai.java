...
public class MuiTenPhai extends HinhThoi {
    {
        maHinh = 13;
    }
    public MuiTenPhai() {
      ...
      public MuiTenPhai(double netVe, Color mauNetVe, Color mauNen) {
        this.netVe = netVe;
    ...
    public void veHinh(double x, double y, double w, double h) {
         mau_Net_Ve();
        ((Polygon) (hinh)).getPoints().setAll(x + 0.5 * w, y + h, x + 0.5 * w, y + 0.75 * h,
                x, y + 0.75 * h, x, y + 0.25 * h, x + 0.5 * w, y + 0.25 * h, x + 0.5 * w, y, x + w, y + 0.5 * h);
    }
